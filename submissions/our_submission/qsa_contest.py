"""
QSA Contest Placer — Standalone Single-File Submission
=======================================================

Self-contained submission for the Partcl/HRT Macro Placement Challenge 2026.
No imports from other submission files — runs as a single file.

Key techniques
--------------
1. Smart init from benchmark.macro_positions (hand-crafted layout).
2. Proxy-cost objective in SA (WL + 0.5*density + 0.5*congestion).
3. Fully vectorised HPWL (~38x faster than Python net-loop).
4. Block translation move (BTM): coordinated cluster moves.
5. SB-QUBO escape (opt-in, default off — classical reheating is more reliable).
6. Soft macro reoptimisation (disabled by default to avoid timeout risk).
7. Auto-loads PlacementCost from disk when evaluator doesn't pass plc.
8. Time-based proxy recompute adapts to machine speed (ibm17: 166s/call).
...
# ---------------------------------------------------------------------------
# plc auto-loader (for evaluator runs that only pass Benchmark, not plc)
# ---------------------------------------------------------------------------

def _load_plc_for_benchmark(name: str):
    """Best-effort plc loader. Returns PlacementCost or None."""
    try:
    ...
    ng45 = {
        ...
        "mempool_tile": "mempool_tile", "nvdla": "nvdla",
        "ariane133_ng45": "ariane133", "ariane136_ng45": "ariane136",
        "mempool_tile_ng45": "mempool_tile", "nvdla_ng45": "nvdla",
    }
...
class QSAConfig:
    ...
    # block_prob=0.10 cost ~0.5% in score. Disabled by default.
    ...
    # Soft macro optimisation — disabled to avoid timeout risk.
    # plc.optimize_stdcells() takes hours on CPU (ibm10: ~10,700s vs 3,600s limit).
    use_soft_macro_opt: bool = False
    softcell_every_accepts: int = 5000
    softcell_num_steps: int = 30

    # Simulated Bifurcation QUBO escape (opt-in; default off)
    use_sb_escape: bool = False
    sb_trigger_window: int = 1500
    sb_trigger_threshold: float = 0.015
    sb_min_interval: float = 90.0
    sb_k: int = 10
    sb_slots_per_macro: int = 6
    sb_overlap_penalty: float = 10.0
    sb_agents: int = 16
    sb_max_steps: int = 5000
...
class QSAPlacer:
    ...
    def place(self, benchmark: Benchmark, plc=None) -> torch.Tensor:
        ...
        Optional plc: PlacementCost for full proxy cost + soft-macro reopt.
        Auto-loads plc from disk if not provided and not on benchmark.
        """
        ...
        # Disk-load plc when evaluator doesn't pass it
        if plc is None:
        ...
        while True:
            ...
            # Periodic full proxy resync (time-based, adapts to machine speed)
    ...
    def _find_hard_overlaps(self, placement, benchmark):
        ...
        return torch.where(ov.any(dim=1))[0]
    ...
    # =====================================================================
    # Soft macro optimisation (disabled by default via use_soft_macro_opt=False)
    # =====================================================================

    def _optimize_softcells(self, plc, benchmark, placement):
        # Runtime guard: skip if not enough time remains to justify the call.
        # plc.optimize_stdcells() can take hours on CPU — always check budget first.
    ...
    def _precompute_overlap_cache(self, benchmark):
        Hn = benchmark.num_hard_macros
        EPS = 1e-3
        self._oc_hw = benchmark.macro_sizes[:Hn, 0] / 2
        self._oc_hh = benchmark.macro_sizes[:Hn, 1] / 2
        self._oc_eps = EPS
        self._oc_Hn = Hn
