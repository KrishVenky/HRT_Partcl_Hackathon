...
class QSAConfig:
    ...
    density_recompute_interval: int = 2000  # SA moves between full proxy recomputes
...
class QSAPlacer:
    ...
    def place(self, benchmark: Benchmark, plc=None) -> torch.Tensor:
        ...
        # Safety: clamp every macro to canvas bounds. SA's _has_overlap doesn't
        # distinguish out-of-bounds from in-bounds; rare numerical drift can
        # push a macro centre slightly past H - h/2.
        Hn = benchmark.num_hard_macros
        sizes_h = benchmark.macro_sizes[:Hn]
        W = benchmark.canvas_width
        H = benchmark.canvas_height
        best_placement[:Hn, 0] = torch.clamp(
            best_placement[:Hn, 0], min=sizes_h[:, 0] / 2, max=W - sizes_h[:, 0] / 2
        )
        best_placement[:Hn, 1] = torch.clamp(
            best_placement[:Hn, 1], min=sizes_h[:, 1] / 2, max=H - sizes_h[:, 1] / 2
        )

        # Final overlap diagnostic: if SA produced overlaps, fall back to
        # shelf-pack (guaranteed legal). Should never trigger.
        if self._full_placement_has_overlap(best_placement, benchmark):
            if cfg.verbose:
                print(f"  [{benchmark.name}] WARN: best_placement has "
                      f"overlaps. Falling back to greedy init.")
    ...
    def _greedy_row_init(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        for idx in sorted_idx.tolist():
            w = benchmark.macro_sizes[idx, 0].item()
            h = benchmark.macro_sizes[idx, 1].item()
            if x_cur + w > W and x_cur > 0:
                y_cur += row_h + EPS
                x_cur = 0.0
                row_h = 0.0
            placement[idx, 0] = x_cur + w/2
            placement[idx, 1] = y_cur + h/2
            x_cur += w + EPS
            row_h = max(row_h, h)
        return placement
